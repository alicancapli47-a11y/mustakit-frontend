...
const cities = ['Adana','Ankara','Antalya','Aydın','Bursa','Denizli','Diyarbakır','Eskişehir','İstanbul','İzmir','Kocaeli','Konya','Mersin','Muğla','Samsun','Trabzon','Diğer']

export default function UstaKayitPage() {
  const [user, setUser] = useState<any>(null)
  const [form, setForm] = useState({ type: '', bio: '', city: '', phone: '', experience: '', priceMin: '', priceMax: '' })
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault()
    if (!form.type || !form.city) { setError('Uzmanlık alanı ve şehir zorunludur'); return }
    ...
    try {
      ...
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/professionals`, {
        ...
        body: JSON.stringify({ userId: user?.id, type: form.type, bio: form.bio, city: form.city, priceMin: form.priceMin ? Number(form.priceMin) : undefined, priceMax: form.priceMax ? Number(form.priceMax) : undefined }),
      })
  ...
  if (success) return (
    ...
    <div className="min-h-screen bg-surface flex items-center justify-center px-4">
      <div className="bg-white border border-border rounded-2xl shadow-card p-10 max-w-md text-center">
        ...
        <p className="text-muted text-sm mb-4">Ekibimiz bilgilerinizi inceleyecek ve onaylandığında e-posta ile bilgilendirileceksiniz.</p>
    ...
    <main className="min-h-screen bg-surface py-10 px-4">
      <div className="max-w-lg mx-auto">
        <div className="bg-white border border-border rounded-2xl p-8">
          <h1 className="font-head font-extrabold text-2xl mb-2 flex items-center gap-2"><Wrench size={22} className="text-primary" /> Usta / Uzman Kayıt</h1>
          <p className="text-muted text-sm mb-6">Bilgilerinizi doldurun, onaylandıktan sonra iş teklifleri almaya başlayın.</p>
          ...
          <form onSubmit={handleSubmit} className="flex flex-col gap-4">
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <div>
                ...
                <input value={form.experience} onChange={e => setForm({...form, experience: e.target.value})} placeholder="10" className="input" />
              </div>
            </div>
            <div>
              <label className="text-xs font-semibold text-gray-600 mb-1.5 block">Hakkınızda</label>
              <textarea value={form.bio} onChange={e => setForm({...form, bio: e.target.value})} placeholder="Kendinizi ve deneyimlerinizi anlatın..." className="input h-20 resize-none" />
            </div>
            <div className="grid grid-cols-2 gap-3">
              <div>
            ...
            <div className="border border-border rounded-lg p-4">
              ...
              <label className="flex items-center gap-2 cursor-pointer bg-surface hover:bg-surface2 border border-border rounded-lg px-4 py-2.5 transition-colors">
                ...
                <input type="file" accept=".pdf,.jpg,.jpeg,.png" className="hidden" onChange={e => { if (e.target.files?.[0]) setCertFile(e.target.files[0]) }} />
              </label>
            </div>
            <div className="border border-border rounded-lg p-4">
              ...
              <label className="flex items-center gap-2 cursor-pointer bg-surface hover:bg-surface2 border border-border rounded-lg px-4 py-2.5 transition-colors">
                ...
                <input type="file" accept=".pdf,.jpg,.jpeg,.png" className="hidden" onChange={e => { if (e.target.files?.[0]) setIdFile(e.target.files[0]) }} />
              </label>
            </div>
