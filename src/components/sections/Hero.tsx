...
export function Hero() {
  ...
  return (
    <section className="relative w-full overflow-hidden" style={{ height: '560px' }}>
      ...
      <div className="absolute inset-0" style={{ background: 'linear-gradient(to right, rgba(0,0,0,0.78) 0%, rgba(0,0,0,0.45) 55%, rgba(0,0,0,0.1) 100%)' }} />
      <div className="absolute inset-0 flex items-center px-8 md:px-16 max-w-6xl mx-auto w-full left-0 right-0">
        <div className="text-white max-w-xl">
          <div className="inline-flex items-center gap-2 rounded-full px-4 py-1.5 text-xs font-semibold mb-5" style={{ background: 'rgba(242,100,25,0.9)' }}>
            {slide.badge}
          </div>
          ...
          <div className="flex items-center gap-3 flex-wrap">
            <Link href="/giris" className="font-semibold px-6 py-3 rounded-lg text-sm text-white" style={{ background: '#F26419' }}>
              Ücretsiz Başla
            </Link>
            <Link href="/#nasil-calisir" className="font-medium px-6 py-3 rounded-lg text-sm" style={{ background: 'rgba(255,255,255,0.12)', border: '1px solid rgba(255,255,255,0.3)', color: 'white' }}>
              Nasıl Çalışır?
            </Link>
          </div>
        </div>
      ...
      <div className="absolute bottom-6 flex gap-2" style={{ left: '4rem' }}>
        {slides.map((_, i) => (
          <button key={i} onClick={() => setCurrent(i)} style={{ width: i === current ? '28px' : '8px', height: '8px', borderRadius: '4px', background: i === current ? 'white' : 'rgba(255,255,255,0.4)', border: 'none', cursor: 'pointer', transition: 'all 0.3s' }} />
        ))}
      </div>
